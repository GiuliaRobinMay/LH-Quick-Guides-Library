...
TOPICS = [
    ('business',  'Business',                '#2C4DC9'),
    ('nonprofit', 'Nonprofit',               '#E2483A'),
    ('career',    'Career',                  '#2F8F5B'),
...
def download_for(code, drive, pdfs, drv):
    dkey = (code or '').replace(' ', '_')
    if dkey in drive:
        return f"https://drive.google.com/uc?export=download&id={drive[dkey]['id']}"
...
def main():
    ...
    with open(OUT_JS, 'w', encoding='utf-8') as f:
        ...
        app_items = [i for i in items if i['status'] == 'POSTED' and i['topic'] in APP_TOPICS]
        app_data = dict(data, topics=[t for t in data['topics'] if t['key'] in APP_TOPICS], items=app_items)
